# MLOps com IaC Para Automação do Deploy de Modelo de Machine Learning na núvem
...
resource "aws_s3_bucket" "bucket_flask" {
    bucket = "mlops-216989116715-bucket" # Nome único do bucket S3
    
    # Define as tags para identificar o bucket
    tags = {
        Name       = "mlops Bucket"
        Environment= "Projeto"
    }

    # Provisiona um script local para fazer o upload de arquivos para o bucket após criação
    ...
    provisioner "local-exec" {
        when    = destroy
        command = "aws s3 rm s3://mlops-216989116715-bucket/ --recursive"
    }
}
...
resource "aws_instance" "ml_api" {
  ...
  user_data = <<-EOF
              ...
              # Corrige o caminho: sincroniza da raiz do bucket
              sudo aws s3 sync s3://mlops-216989116715-bucket/ /ml_app
    ...
    Statement = [
      {
        ...
        Effect = "Allow",
        Resource = [
          "${aws_s3_bucket.bucket_flask.arn}/*", # Permissões nos objetos do bucket
          "${aws_s3_bucket.bucket_flask.arn}"    # Permissões no bucket
        ]
      },
    ]
  })
